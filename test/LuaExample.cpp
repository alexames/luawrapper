...
static int Example_PrintMe(lua_State* L) {
  ...
  cout << "Example="
       << "{boolean=" << ex->boolean << ",integer=" << ex->integer << ",uinteger=" << ex->uinteger << ",string='" << ex->cstring << "'"
       << ",cppstring='" << ex->cppstring << "'"
       << ",number=" << ex->number << ",floatnumber=" << ex->floatnumber << ",vec={x=" << ex->vec.x << ",y=" << ex->vec.y << "}}" << endl;
  return 0;
}
